...
std::optional<int> FuzzySearch::Score(std::string_view query, std::string_view candidate) {
    if (query.empty() || candidate.empty()) return std::nullopt;

    const std::string normalizedQuery = Lower(query);
    const std::string normalizedCandidate = Lower(candidate);

    if (normalizedQuery == normalizedCandidate) return 2000;

    int score = 0;
    if (normalizedCandidate.starts_with(normalizedQuery)) score += 500;
    ...
    for (std::size_t candidateIndex = 0;
        ...
        score += 10;
        if (IsBoundary(normalizedCandidate, candidateIndex)) score += 25;
    ...
    if (queryIndex != normalizedQuery.size()) return std::nullopt;

    score -= std::min(40, static_cast<int>(normalizedCandidate.size() - normalizedQuery.size()));
    return score;
}
