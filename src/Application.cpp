...
constexpr std::array<CommandDefinition, 12> Commands{{
    ...
    {"color", "set task foreground/background RGB"},
...
void Application::PeriodicAutosave() {
    ...
    if (now - lastAutosave_ < std::chrono::seconds(1)) {
        return;
    }
...
void Application::ApplyColors() {
    ...
    // Keep the rest of Sentinel in the terminal's normal colors. The custom
    // pair is activated only while task rows are rendered.
    bkgd(A_NORMAL);
    attrset(A_NORMAL);
}
...
void Application::HandleMouse() {
    ...
    if (event.y < startRow ||
        event.y >= startRow + static_cast<int>(suggestions.size())) {
        return;
    }
...
std::vector<Application::Suggestion> Application::BuildSuggestions() const {
    ...
    if (separator == std::string::npos) {
        ...
        for (const auto& definition : Commands) {
            ...
            if (score) {
                ranked.push_back({*score, definition});
            }
        }
...
void Application::RenderTasks() {
    ...
    for (const std::size_t index : VisibleTaskIndices()) {
        ...
        if (has_colors()) {
            attron(COLOR_PAIR(TaskColorPair));
        }

        mvaddnstr(row++, 0, line.str().c_str(), std::max(0, width - 1));

        if (has_colors()) {
            attroff(COLOR_PAIR(TaskColorPair));
        }
    }
...
void Application::RenderCommandLine() {
    ...
    move(
        ...
        std::min(
            width - 1,
            static_cast<int>(commandBuffer_.size()) + 2
        )
    );
}
